...
# Code for Part Two ----------------------------------------------------
def partTwo(inputFilePath, lowest):
  ...
  successfulReindeer:list[Reindeer] = []
  lowestToLoc:list[Reindeer] = []
  ...
  while (reindeers):
    ...
    try:
      ...
      if reindeer.score < lowestSoFar.score:
        lowestToLoc[ind] = reindeer
      elif reindeer.score == lowestSoFar.score:
        lowestToLoc[ind].prev = list(set(reindeer.prev).union(set(lowestSoFar.prev)))
      else:
        continue
    except ValueError:
      lowestToLoc.append(reindeer)
    ...
    if map[reindeer.x][reindeer.y] == 'E':
      successfulReindeer.append(lowestToLoc[lowestToLoc.index(reindeer)])
      continue
  ...
  tilesOnShortestPaths = set()
  for reindeer in successfulReindeer:
    for tile in reindeer.prev:
      tilesOnShortestPaths.add(tile)

  return len(tilesOnShortestPaths) + 1
...
# answer = partTwo("day16/test.txt", 11048)
answer = partTwo("day16/input.txt", 133584)
